...
func (h humanRenderer) report(r *Report) {
	...
	h.fact("Auth", authStateLabel(r.Target.AuthState))
	if c := r.Target.Capabilities; c != nil {
		surface := fmt.Sprintf("%d tools  •  %d resources  •  %d prompts", c.Tools, c.Resources, c.Prompts)
		if c.Truncated {
			surface += "  (truncated — counts are a floor)"
		}
		h.fact("Surface", surface)
	}
	...
	if r.Status == StatusIncomplete {
		fmt.Fprintf(h.w, "\n %s\n", h.paint(ansiBold+ansiYellow, "⚠ SCAN INCOMPLETE — ABSENT FINDINGS ARE NOT CLEAN RESULTS"))
	...
	coverage := fmt.Sprintf("%d checks  •  %d matched  •  %d clean  •  %d skipped",
		r.Coverage.Total, r.Coverage.Fired, r.Coverage.Clean, r.Coverage.Skipped)
	if r.Coverage.Errored > 0 {
		coverage += fmt.Sprintf("  •  %s", h.paint(ansiYellow, fmt.Sprintf("%d could not run", r.Coverage.Errored)))
	}
	fmt.Fprintf(h.w, "\n %s  •  %s\n", coverage, duration)
	fmt.Fprintf(h.w, " %s\n", r.summaryLine())
	for _, p := range r.Probes {
		...
		fmt.Fprintf(h.w, " %s %s (%s): %s\n", h.paint(ansiYellow, "!"), p.ID, p.Status, p.Detail)
	}
...
// authStateLabel renders Target.AuthState for the identification block. An
// empty or unknown state renders as nothing rather than as a misleading
// "unknown", since h.fact skips empty values.
func authStateLabel(state string) string {
	switch state {
	case AuthStateOpen:
		return "open  •  enumeration answered without credentials"
	case AuthStateGated:
		return "auth-gated  •  live, but requires credentials to enumerate"
