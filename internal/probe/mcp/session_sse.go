...
func NewSSESession(sseURL, authHeader string, client *httpx.Client) (*SSESession, error) {
	// The GET stream is intentionally long-lived — per-request deadlines are
	// enforced via the ctx each Do() call receives, not a client-wide timeout
	// that would kill the SSE connection itself. Everything else (proxy, TLS
	// verification, User-Agent, rate limit) comes from the shared client.
