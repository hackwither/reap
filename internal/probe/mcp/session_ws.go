...
// frame reader.
func NewWSSession(targetURL, authHeader string, client *httpx.Client) (*WSSession, error) {
	// A raw socket upgrade has no proxy semantics, so only the timeout
	// carries over from the shared client.
