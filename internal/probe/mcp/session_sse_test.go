...
// mustSSESession builds an SSE session with the shared HTTP client, failing
// the test rather than returning an error to every call site.
func mustSSESession(t *testing.T, url string) *SSESession {
	t.Helper()
	sess, err := NewSSESession(url, "", testClient(t))
	if err != nil {
		t.Fatalf("NewSSESession: %v", err)
	}
	return sess
}
