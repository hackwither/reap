...
func TestNewSessionForTransport_DispatchesByTransport(t *testing.T) {
	...
	for _, transport := range []string{"http-streamable", "", "some-future-transport"} {
		sess, err := newSessionForTransport("mcp", "http://example.invalid/mcp", transport, "", testHTTPClient(t))
		if err != nil {
	...
	sess, err := newSessionForTransport("mcp", "http://example.invalid/sse", "http-sse-legacy", "", testHTTPClient(t))
	if err != nil {
...
func TestNewSessionForTransport_WebSocketDialsImmediately(t *testing.T) {
	...
	sess, err := newSessionForTransport("mcp", wsURL, "websocket", "", testHTTPClient(t))
	if err != nil {
...
func TestNewSessionForTransport_WebSocketErrorPropagates(t *testing.T) {
	...
	_, err := newSessionForTransport("mcp", wsURL, "websocket", "", testHTTPClient(t))
	if err == nil {
		t.Fatal("expected an error when the server never completes the WebSocket upgrade")
	}
}

// testHTTPClient builds the shared outbound client the way cli.Run does.
func testHTTPClient(t *testing.T) *httpx.Client {
	t.Helper()
	c, err := httpx.New(httpx.Config{Timeout: time.Second}, version.UserAgent())
	if err != nil {
...
// TestNewSessionForTransport_NonMCPGetsGenericSession covers the breadth
// seam: a protocol reap can identify but not enumerate must still get a
// session, so the protocol-neutral transport checks produce a real report
// instead of the target being dropped after a failed MCP handshake.
func TestNewSessionForTransport_NonMCPGetsGenericSession(t *testing.T) {
	for _, protocol := range []string{"a2a", "openapi"} {
		sess, err := newSessionForTransport(protocol, "http://example.invalid/.well-known/agent.json", "http-json", "", testHTTPClient(t))
		if err != nil {
			t.Fatalf("%s: unexpected error: %v", protocol, err)
		}
		if _, ok := sess.(*generic.Session); !ok {
			t.Fatalf("%s: expected *generic.Session, got %T", protocol, sess)
		}
	}
}
